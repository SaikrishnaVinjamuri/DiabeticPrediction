...
class ModifiedFireflyAlgorithm():
    ...
    def __init__(self, function):
        self.D = 8  # dimension of the problem (Gene number)
        self.NP = 1  # population size (Firefly number)
    ...
    def Run(self):
        while self.evaluations < self.nFES:
            ...
            for i in range(self.NP):
                ...
                print(str(self.evaluations)+" "+str(self.nFES)+" "+str(self.Fitness[i]))
...
def evaluation(feature_possibilities):
        ...
        s = RandomForestClassifier()#svm.SVC(kernel="poly", C=1)
...
def runModifiedFirefly(XX, YY):
    global X, y
    X = XX
    y = YY
    Algorithm = ModifiedFireflyAlgorithm(evaluation)
    Best = Algorithm.Run()
    a = np.round(Best)
    best_features = a > np.float32(0.4)
    np.save("model/modified_firefly", best_features)
